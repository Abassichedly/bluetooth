...
class _DevicesState extends State<Devices> {
  ...
  List<Map<String, dynamic>> lastScannedDevices =
      []; // List to store last scanned devices with UID and status
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        scannedDevices.forEach((device) {
                          if (!lastScannedDevices.any(
                              (Map<String, dynamic> item) =>
                                  item['device'].id == device.id)) {
                            lastScannedDevices.add({
                              'device': device,
                              'uid':
                                  '', // Assuming uid is empty here, replace it with the actual value if needed
                            ...
                            Column(
                              children: [
                                ...
                                ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    final deviceDetails =
                                        lastScannedDevices[index];
                                    final BluetoothDevice device =
                                        deviceDetails['device'];
                                    final String uid = deviceDetails['uid'];
                                    final String status = deviceDetails[
                                        'status']; // Get status directly from the map
                                    return GestureDetector(
                                      onTap: () => _showAssociationDialog(
                                          context, device, controller),
                                      child: Card(
                                        elevation: 2,
                                        child: ListTile(
                                          ...
                                          subtitle: Column(
                                            ...
                                            children: [
                                              ...
                                              Row(
                                                children: [
                                                  Text(
                                                      'Status: $status'), // Use status from the map directly
                                                  SizedBox(width: 5),
                                                  Icon(
                                                    status == 'connected'
                                                        ? Icons.check_circle
  ...
  Future<String> _showAssociationDialog(BuildContext DevicesContext,
      BluetoothDevice device, BluetoothController controller) async {
        ...
        try {
          ...
          stateSubscription = device.state.listen((state) {
            ...
            if (state == BluetoothDeviceState.connected) {
              print('ok');
              controller.updateAssociationResponse(
                  "Device ${device.name} associated successfully");
              // Update status in lastScannedDevices
              final deviceIndex = lastScannedDevices
                  .indexWhere((item) => item['device'].id == device.id);
              if (deviceIndex != -1) {
                setState(() {
              ...
              controller.updateAssociationResponse(
                  "Failed to associate with device ${device.name}");
              // Update status in lastScannedDevices
              final deviceIndex = lastScannedDevices
                  .indexWhere((item) => item['device'].id == device.id);
              if (deviceIndex != -1) {
              ...
              if (service.uuid.toString() == UuidHelper.lightServiceUuid) {
                service.characteristics.forEach((characteristic) {
                  if (characteristic.uuid.toString() ==
                      UuidHelper.lightCharacteristicUuid) {
  ...
  Future<void> sendDevicesToBackend(List<Map<String, dynamic>> devices) async {
    ...
    try {
      ...
      List<Map<String, dynamic>> devicesJson = devices
          .map((device) => {
                ...
                'status': device['status'] != null
                    ...
                    : 'unknown', // Ensure status is not null
              })
          .toList();
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "Failed to update devices: ${response.statusCode} , ${response.body}");
      }
    } catch (e) {
